  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column', width: '100%', background: 'var(--bg-primary)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Header Navigation */}
        <header style={{ padding: '1.5rem 2rem 0', width: '100%', maxWidth: '1200px', margin: '0 auto' }}>
          <button
            onClick={() => router.back()}
            style={{
        ...
        {/* Main Content Area */}
        <main style={{ flex: 1, padding: '1rem 2rem 6rem', maxWidth: '1200px', width: '100%', margin: '0 auto' }}>
          
          {/* Bio Text */}
          <div style={{ color: '#fff', fontSize: '1rem', lineHeight: '1.6', marginBottom: '2.5rem', background: 'rgba(255,255,255,0.02)', padding: '15px', borderRadius: '4px', borderLeft: '4px solid #22C55E' }}>
            {isAlphaBoy ? (
              <p style={{ margin: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'stretch' }}>
            ...
            {/* Left Column: Achievements Section */}
            <div style={{ flex: '1', minWidth: '300px', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ border: '1px solid #3c3c3c', borderBottomLeftRadius: '8px', borderBottomRightRadius: '8px', overflow: 'hidden', background: '#1a1a1a', flex: 1 }}>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={idx} style={{ borderBottom: '1px solid #3c3c3c', background: idx % 2 === 0 ? 'rgba(255,255,255,0.01)' : 'transparent' }}>
                        <td style={{ padding: '12px 15px', fontSize: '0.95rem', borderRight: '1px solid #3c3c3c', color: '#eee' }}>{ach.date}</td>
                        <td style={{ padding: '12px 15px', borderRight: '1px solid #3c3c3c', textAlign: 'center' }}>
                          <span style={{ 
                            ...
                            fontWeight: 800,
                            textAlign: 'center',
                            background: ach.place === '1st' ? '#a08a00' : ach.place === '3rd' ? '#4d4d4d' : '#2c5f63',
                            color: '#fff'
                          }}>
                            {ach.place}
                          </span>
                        ...
                        <td style={{ padding: '12px 15px', borderRight: '1px solid #3c3c3c' }}>
                          <span className="tourney-link" style={{ fontSize: '0.95rem', color: '#fff', fontWeight: 500, cursor: 'pointer', transition: 'color 0.2s ease' }}>
                            {ach.tourney}
                          </span>
                        </td>
                        <td style={{ padding: '12px 15px', fontSize: '0.9rem', color: '#3498db', fontWeight: 700 }}>{ach.team}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ 
              ...
              {/* Infobox Header: Centered Name, Logo Left */}
              <div style={{ 
                background: '#1a1a1a', 
                padding: '12px 15px', 
                display: 'flex', 
                ...
                <span style={{ fontWeight: 800, fontSize: '1.1rem', color: '#fff', letterSpacing: '-0.02em' }}>AlphaBoy</span>
              </div>

              {/* Player Pic */}
              <div style={{ width: '100%', background: '#000', display: 'flex', justifyContent: 'center', borderBottom: '1px solid #3c3c3c' }}>
                <NextImage 
                  src="/Alphaboy-EsportPic.jpg" 
                  alt="AlphaBoy" 
                  width={280} 
                  height={350} 
              ...
              {/* Player Information Section Header */}
              <div style={{ background: 'linear-gradient(90deg, #1a1a1a 0%, #2c2c2c 100%)', padding: '10px 10px', textAlign: 'center', borderBottom: '1px solid #3c3c3c' }}>
                <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800, color: '#22C55E', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Player Information</h3>
              </div>

              {/* Player Info Details Table */}
                ...
                <tbody>
                  {[
                    ...
                    <tr key={idx} style={{ background: idx % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent' }}>
                      <td style={{ padding: '8px 12px', fontWeight: 700, color: '#9CA3AF', width: '42%', textAlign: 'right', borderRight: '1px solid #3c3c3c', whiteSpace: 'nowrap' }}>{info.label}</td>
                      <td style={{ padding: '8px 12px', fontWeight: 800, color: info.color || '#fff', textAlign: 'left', whiteSpace: 'nowrap' }}>{info.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
